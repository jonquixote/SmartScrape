...
class AIGuidedStrategy(BaseStrategy):
    ...
    def can_handle(self, url: str, **kwargs) -> bool:
        """
        Check if the AI-guided strategy can handle the given URL.
        ...
        Returns:
            bool: True if the strategy can handle the URL, False otherwise
        """
        try:
            ...
            # Exclude problematic file types that AI can't effectively process
            excluded_extensions = {'.pdf', '.doc', '.docx', '.xls', '.xlsx', '.zip', '.exe', '.dmg', '.mp4', '.avi', '.mov'}
            for ext in excluded_extensions:
                if url_lower.endswith(ext):
                    return False
            
            # AI-guided strategy can handle most web URLs, especially complex ones
            return True
            
        except Exception as e:
            logger.warning(f"Error checking if AI-guided strategy can handle URL {url}: {e}")
            return False
    ...
    async def execute(self, url: str, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        Args:
            ...
            **kwargs: Additional parameters including search_terms, extraction_config, crawler
        
        Returns:
            Dictionary containing structured results or None if failed
        """
        ...
        logger.info(f"Using global timeout of {timeout_seconds} seconds for {url}")
        ...
        try:
            ...
            task = asyncio.create_task(self._execute_inner(crawler, url, extraction_config))
            ...
            try:
                ...
                logger.error(f"AI-guided strategy timed out after {timeout_seconds} seconds for {url} during {current_phase} phase")
    ...
    async def _execute_inner(self, crawler, url, extraction_config=None):
        """
        ...
        """
        logger.info(f"Executing AI-guided strategy for URL: {url}")
        ...
        try:
            ...
            try:
                ...
                if hasattr(self, 'cancel_execution') and self.cancel_execution:
                    logger.warning(f"Execution cancelled for {url}")
                    return {"cancelled": True, "url": url, "timestamp": time.time()}
                ...
                try:
                    ...
                    search_form_task = self._find_and_use_search_forms(url, search_terms)
                ...
                if search_result and search_result.get('success') and search_result.get('html_content'):
                    ...
                    try:
                        ...
                        logger.info(f"Direct request to {url} succeeded with status {direct_response.status_code}")
                    ...
                    if not response or not hasattr(response, 'text'):
                        logger.error(f"Invalid response from _make_request for URL: {url}")
                        raise ValueError(f"Invalid response from _make_request for URL: {url}")
                    html_content = response.text
                    current_url = url  # Track the URL we're actually processing
                    logger.info(f"Successfully retrieved HTML content ({len(html_content)} bytes) from {url}")
                ...
                if hasattr(self, 'user_prompt') and self.user_prompt:
                    ...
                    if property_listings and len(property_listings) > 0:
                        ...
                        for idx, listing in enumerate(property_listings):
                            ...
                            self.add_result(url, listing_result, 0)
                ...
                if not property_listings or len(property_listings) == 0:
                    ...
                    try:
                        ...
                        extraction_task = self._extract_with_ai_guidance(html_content, url, self.user_prompt, self.config)
                        ...
                        if result:
                            # Add to results
                            self.add_result(url, result, 0)
                            logger.info(f"Successfully extracted data from {url} using AI guidance")
                            ...
                            logger.warning(f"No data extracted from {url} using AI guidance")
                    except asyncio.TimeoutError:
                ...
                logger.warning(f"Circuit breaker prevented request to {url}: {str(e)}")
                
                # Try to use cached results or fallback method
                if self.config.get('fallback_to_dom', True):
                    logger.info(f"Using DOM strategy as fallback for {url}")
                    
                    # Update progress
                    if hasattr(self, 'execution_progress'):
                        self.execution_progress['phase'] = 'fallback_dom_strategy'
                    
                    dom_result = await self.dom_strategy.execute(url, crawler=crawler, extraction_config=extraction_config)
                    return dom_result
    ...
    def crawl(self, url: str, **kwargs) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        try:
            logger.info(f"AI-guided crawl starting from: {url}")
            ...
            result = self.execute(url, **kwargs)
            ...
            if result and result.get("success"):
                ...
                logger.warning(f"AI-guided crawl failed for {url}")
                return None
